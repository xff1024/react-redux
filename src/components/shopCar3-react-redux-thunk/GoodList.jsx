...
// 模拟异步操作
import {asyncAddAction} from './store/actionCreator'
                    ...
                    {this.state.fruitList.map(item => {
                        return (
                            <li key={item.id}>
                                ...
                                <p>{item.name}</p>
                                <div>
                                    <Button type="success" onClick={this.props.asyncAddGoods.bind(this,item)}>加入购物车</Button>
                                </div>
                            </li>
                        )
                    })}
...
// 将修改仓库中的值挂载到组件的属性中，通过 this.props.fn() 调用
